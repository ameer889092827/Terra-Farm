...
import { GameState, TileState, NasaData, Tool, Season, Crop, Achievement, NotificationMessage, Location, SoilType, FarmAnalysis } from './types';
...
import { getFarmAnalysis } from './services/geminiService';
...
import NASADataGuideModal from './components/NASADataGuideModal';
...
const App: React.FC = () => {
  ...
  const [showNASAWorldMap, setShowNASAWorldMap] = useState<boolean>(false);
  const [showLearningModules, setShowLearningModules] = useState<boolean>(false);
  ...
  const [showNASADataGuide, setShowNASADataGuide] = useState<boolean>(false);
  const [showScenario, setShowScenario] = useState<boolean>(false);
  const [currentScenario, setCurrentScenario] = useState<'heat_wave' | 'sensor_malfunction' | 'pest_outbreak' | null>(null);

  const [isAiLoading, setIsAiLoading] = useState<boolean>(false);
  const [inspectedTileIndex, setInspectedTileIndex] = useState<number | null>(null);
  const [farmAnalysis, setFarmAnalysis] = useState<FarmAnalysis | null>(null);
  ...
  const handleAction = useCallback((index: number) => {
    ...
    switch (gameState.selectedTool) {
      case 'plant':
        ...
        break;
      case 'water':
        const waterCost = 10 * (gameState.upgrades.water_cost_multiplier || 1);
  ...
  const handleRequestAnalysis = useCallback(async () => {
    ...
    try {
        const analysis = await getFarmAnalysis(gameState, nasaData, selectedLocation);
        setFarmAnalysis(analysis);
    } catch (error) {
        console.error("Error fetching Terra analysis:", error);
        setFarmAnalysis({
          summary: "My analysis systems are down. What does the raw data tell you?",
          recommendations: ["- Check soil moisture levels manually.", "- Monitor plant health for visible stress signs."]
        });
    } finally {
  ...
  useEffect(() => {
    ...
    const scenarioTimer = setInterval(() => {
      if (Math.random() < 0.50) { // 20% chance every 30 seconds (increased from 5%)
        ...
        showNotificationMessage(`Educational scenario: ${randomScenario.replace('_', ' ')}`, 'info');
      }
    }, 30000);
    
    return () => clearInterval(scenarioTimer);
  }, [gameState, nasaData, showNotificationMessage]);
  ...
  const handleBuyUpgrade = useCallback((upgradeId: string, cost: number) => {
    setGameState(prev => {
        ...
        if (!upgrade || prev.purchasedUpgradeIds.includes(upgradeId)) {
            showNotificationMessage("Upgrade not found or already purchased!", 'warning');
            return prev;
        }
        ...
        const newState: GameState = {
            ...
            upgrades: { ...prev.upgrades, [upgrade.stat]: upgrade.modifier },
            purchasedUpgradeIds: [...prev.purchasedUpgradeIds, upgradeId],
        };
  ...
  const startGame = () => {
    ...
    handleRequestAnalysis(); // Get initial analysis instead of a simple tip
  };
    ...
    <div className="bg-slate-900 min-h-screen flex items-center justify-center p-4 text-white font-sans antialiased">
      <div className="w-full max-w-7xl h-[800px] max-h-[90vh] bg-slate-800 rounded-2xl shadow-2xl shadow-cyan-500/10 flex flex-col overflow-hidden ring-1 ring-slate-700">
        <Header 
            ...
            onNASAWorldMapClick={() => setShowNASAWorldMap(true)}
            onLearningModulesClick={() => setShowLearningModules(true)}
            onWeatherForecastClick={() => setShowWeatherForecast(true)}
            onResearchLabClick={() => setShowResearchLab(true)}
            onNASADataGuideClick={() => setShowNASADataGuide(true)}
        />
      ...
      {showNASAWorldMap && <NASAWorldMap onClose={() => setShowNASAWorldMap(false)} />}
      {showLearningModules && <LearningModules onClose={() => setShowLearningModules(false)} selectedCrop={gameState.tiles.find(t => t.planted)?.crop} nasaData={nasaData} />}
      {showWeatherForecast && <WeatherForecast currentNasaData={nasaData} location={selectedLocation} onClose={() => setShowWeatherForecast(false)} />}
      {showResearchLab && <ResearchLab onClose={() => setShowResearchLab(false)} availableCrops={gameState.availableCrops} nasaData={nasaData} />}
      {showNASADataGuide && <NASADataGuideModal onClose={() => setShowNASADataGuide(false)} />}
      {showScenario && currentScenario && <ScenarioModal scenario={currentScenario} onClose={() => { setShowScenario(false); setCurrentScenario(null); }} />}
    </div>
  );
};
